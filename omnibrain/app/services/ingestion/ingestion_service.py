...
# Robust imports matching your actual directory layout
try:
    from omnibrain.app.services.ingestion.text_parser import extract_text_and_chunk
except ImportError:
    try:
        from scripts.prototypes.pdf_text_proto import parse_pdf as extract_text_and_chunk
    except ImportError:
        def extract_text_and_chunk(pdf_path: str) -> List[Dict[str, Any]]:
            ...
            return [
                {
                    "chunk_id": "chunk_0",
                    "text": "Sample text content extracted from document.",
                    "page_number": 1,
                    "metadata": {"source": pdf_path, "section": "General"},
                }
            ]
...
try:
    from omnibrain.vectorstore.indexers import index_image_vectors
except ImportError:
...
class IngestionService:
    ...
    Coordinates Text Parsing -> Vision Extraction -> Embedding Generation -> Vector DB Storage.
    """
    ...
    def process_pdf(self, file_path: str, source_filename: str = None) -> IngestionResponse:
        ...
        # 4. Construct Final IngestionResponse matching exact schema fields
        total_pages = max(
            [c.page_number for c in validated_text_chunks] + [img.page_number for img in validated_images] + [1]
        )
        ...
        # Bypass Pydantic attributes and log the local raw lengths directly
        logger.info(
            f"Ingestion completed for '{filename}': "
            f"{len(validated_text_chunks)} text chunks, {len(validated_images)} images stored."
        )
        return response
    ...
    def _validate_text_chunks(self, raw_chunks: List[Any], filename: str) -> List[TextChunk]:
        """Converts raw dictionaries/objects safely into typified TextChunk Pydantic models."""
        validated = []
    ...
    def _validate_images(self, raw_images: List[Any], filename: str) -> List[ExtractedImage]:
        """Converts raw dictionaries/objects safely into typified ExtractedImage Pydantic models."""
        validated = []
    ...
    def _persist_to_vector_store(self, text_chunks: List[TextChunk], images: List[ExtractedImage]) -> None:
        if text_chunks:
